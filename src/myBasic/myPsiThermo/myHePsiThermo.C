...
void Foam::myHePsiThermo<BasicPsiThermo, MixtureType>::calculate
(
...
)
{
    ...
    forAll(TCells, celli)
    {
        ...
        if (this->updateT())
        {
            ...
            if(TCells[celli]<200 || TCells[celli]>5000)
            {
                Info << "TCells[" << celli << "] = " << TCells[celli] << endl;
                Info << "pCells[" << celli << "] = " << pCells[celli] << endl;
                Info << "hCells[" << celli << "] = " << hCells[celli] << endl;
                Info << "mixture_ = " << this->cellMixture(celli) << endl;
            }
        }
...
template<class BasicPsiThermo, class MixtureType>
Foam::myHePsiThermo<BasicPsiThermo, MixtureType>::myHePsiThermo
(
    const fvMesh& mesh,
...
template<class BasicPsiThermo, class MixtureType>
void Foam::myHePsiThermo<BasicPsiThermo, MixtureType>::correct()
{
    DebugInFunction << endl;

    Info << "myHeReactionThermo.H 4 " << endl;
