...
#include "mySingleStepReactingMixture.H"
#include "fvMesh.H"

// * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * * //

template<class ThermoType>
void Foam::mySingleStepReactingMixture<ThermoType>::calculateqFuel()
{
    ...
    forAll(reaction.lhs(), i)
    {
        ...
        Info << "speciei = " << speciei << endl;
        Info << "stoichCoeff = " << stoichCoeff << endl;
        specieStoichCoeffs_[speciei] = -stoichCoeff;
        Info << "stspecieStoichCoeffs_[speciei] = " << specieStoichCoeffs_[speciei] << endl;
    ...
    Info<< "Fuel heat of combustion 0 :" << qFuel_.value() << endl;
    Info<< "Fuel heat of combustion :" << qFuel_.value() << endl;
}


template<class ThermoType>
void Foam::mySingleStepReactingMixture<ThermoType>::massAndAirStoichRatios()
{
...
template<class ThermoType>
void Foam::mySingleStepReactingMixture<ThermoType>::calculateMaxProducts()
{
    const Reaction<ThermoType>& reaction = this->operator[](0);
...
template<class ThermoType>
void Foam::mySingleStepReactingMixture<ThermoType>::fresCorrect()
{
    const Reaction<ThermoType>& reaction = this->operator[](0);
...
template<class ThermoType>
Foam::mySingleStepReactingMixture<ThermoType>::mySingleStepReactingMixture
(
...
template<class ThermoType>
void Foam::mySingleStepReactingMixture<ThermoType>::read
(
    const dictionary& thermoDict
)
{}
